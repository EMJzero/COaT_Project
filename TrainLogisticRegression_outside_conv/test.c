...
void gradient_descent(int n, float *w, float *s, float *o) {
    ...
    for (int epoch = 0; epoch < EPOCHS; epoch++) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < SIZE; j++) {
                w[j] -= LEARNING_RATE * (predicted - o_) * s[j];
            }
        }
    }
}
...
int main() {
    ...
    float w[SIZE] __attribute((annotate("target('weights') scalar(range(-2, 2))"))) = {0.0}; // Initial weights (set to 0 initially)
    float s[SIZE * 100] __attribute((annotate("scalar(range(-0.1, 1.1) final)"))); // Input samples, SIZE features per sample, 100 samples
    float o[100] __attribute((annotate("scalar(range(-0.1, 1.1) final)"))); // Correct output samples for each batch of SIZE elements of *s
